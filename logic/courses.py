...
class Course :
    ...
    # Métodos setter para asignar los datos de un curso
    ...
    def getDays(self):
        return {
            day: {
                'salon': day_obj.getClassroom(), 
                'hora_inicio': day_obj.getStart(), 
                'hora_fin': day_obj.getEnd()
            }
    ...
    def update_day(self, day, classroom, start, end):
        """Método para agregar o modificar los días de clase."""
        normalized_day = self.__normalized_day(day)
        if self.__valid_day(day):
            self.days[normalized_day] = Day(normalized_day, classroom, start, end)
        else:
